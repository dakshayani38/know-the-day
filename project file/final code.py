...
def web_scraping():
    ...
    html_nd=requests.get("https://nationaldaycalendar.com/" +listofmonths[int(monthfromentry.get())-1]+"/"+"#tab-"+str(datefromentry.get()),headers={'User-Agent': 'Mozilla/5.0'}).text
    parsed_nd=BeautifulSoup(html_nd, 'lxml')
    required_div_tag=parsed_nd.find('div',class_="kt-tabs-content-wrap")
    all_h2_tags=required_div_tag.find_all('h2')

    for each_h2_tag in all_h2_tags:
        if str(today.day) in each_h2_tag.text:
            list_of_days.append(" ".join(each_h2_tag.text.split()[:-3]))
        if str(today.day+1) in each_h2_tag.text:
            break
...
def popupwindow():
    ...
    top_window_input.iconbitmap('KD icon.ico')
...
up_photo = Image.open("up-arrow.jpg")
...
down_photo = Image.open("down-arrow.jpg")
